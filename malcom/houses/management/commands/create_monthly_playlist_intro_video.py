"""
Creates an intro video for a MonthlyPlaylist.
...
Usage:
    python manage.py create_monthly_playlist_intro_video <playlist_id> [--output OUTPUT_PATH]
"""
...
from houses.models import MonthlyPlaylist, PerformanceSchedule
from PIL import Image, ImageDraw, ImageFont

logger = logging.getLogger(__name__)

# Video settings
VIDEO_WIDTH = 1920  # noqa: N806
VIDEO_HEIGHT = 1080  # noqa: N806
VIDEO_FPS = 30  # noqa: N806
...
TITLE_FONT_SIZE = 80  # noqa: N806
ARTIST_FONT_SIZE = 60  # noqa: N806
INFO_FONT_SIZE = 40  # noqa: N806
SMALL_FONT_SIZE = 30  # noqa: N806
...
def create_artist_slide(  # noqa: C901, PLR0912, PLR0915, PLR0911
    artist_name: str,
    song_title: str,
...
class Command(BaseCommand):
    help = "Create an intro video for a monthly playlist"

    def add_arguments(self, parser: CommandParser):
        parser.add_argument(
            "playlist_id",
            type=int,
            help="ID of the MonthlyPlaylist to create video for",
        )
        parser.add_argument(
            "--output",
            "-o",
    ...
    def handle(self, *args, **options):  # noqa: ANN002, ANN003  # noqa: C901, PLR0915
        ...
        except MonthlyPlaylist.DoesNotExist:  # noqa: DJ012
            self.stderr.write(self.style.ERROR(f"MonthlyPlaylist with ID {playlist_id} not found"))
            return

        self.stdout.write(f"Creating intro video for playlist: {playlist.date}")

        # Get playlist entries
        entries = playlist.monthlyplaylistentry_set.select_related("song__performer").order_by("position")
        ...
        clips = []
        playlist_title = f"HAKKO-AKKE {playlist.date.strftime('%B')} [TOKYO] {playlist.date.strftime('%Y')}"
        ...
        for entry in entries:
            ...
            # Find upcoming performances for this performer
            # Look for performances in the playlist's month and next month
            month_start = playlist.date
            month_end = playlist.date.replace(day=28) + timedelta(days=4)
            month_end = month_end.replace(day=1) + timedelta(days=31)
        ...
        output_filename = f"playlist_{playlist_id}_{playlist.date.strftime('%Y%m')}_intro.mp4"
