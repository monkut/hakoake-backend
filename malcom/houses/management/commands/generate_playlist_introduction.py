"""Generate playlist introduction text and audio for a given MonthlyPlaylist."""
...
from houses.functions import ROBOTIC_VOICE_PRESETS, generate_playlist_introduction_text, generate_robotic_tts
from houses.models import MonthlyPlaylist


class Command(BaseCommand):
    help = "Generate introduction text and optional robotic TTS audio for a monthly playlist using AI"

    def add_arguments(self, parser: CommandParser) -> None:
        parser.add_argument(
            "playlist_id",
            type=int,
            help="ID of the MonthlyPlaylist to generate introduction for",
        )
        parser.add_argument(
            "--voice",
            type=str,
    ...
    def handle(self, *args, **options) -> None:  # noqa: ANN002, ANN003
        ...
        self.stdout.write(f"Generating introduction for playlist: {playlist.date.strftime('%B %Y')}")
        self.stdout.write(f"Playlist URL: {playlist.youtube_playlist_url}")
        self.stdout.write("")

        # Generate introduction text
        result_introduction, _ = generate_playlist_introduction_text(playlist)
        ...
        if generate_audio:
            ...
            audio_filename = f"playlist_{playlist.id}_{playlist.date.strftime('%Y%m')}_{voice_preset}.mp3"
